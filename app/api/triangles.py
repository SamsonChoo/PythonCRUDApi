...
def create_triangle():
    ...
    if type(data['length1']) != int or type(data['length2']) != int or type(data['length3']) != int:
        return bad_request('length must be integers')
...
def update_triangle(triangle_id):
    ...
    data = request.get_json() or {}
    if 'length1' in data and type(data['length1']) != int or 'length2' in data and type(data['length2']) != int or 'length3' in data and type(data['length3']) != int:
        return bad_request('length must be integers')
